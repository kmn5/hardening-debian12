...
DESCRIPTION="6.3.3.18 - Ensure successful and unsuccessful attempts to use the usermod command are recorded"

PACKAGE='auditd'
UID_MIN=$(awk '/^\s*UID_MIN/{print $2}' /etc/login.defs)
AUDIT_PARAMS="-a always,exit -F path=/usr/sbin/usermod -F perm=x -F auid>=$UID_MIN -F auid!=unset -k usermod"
