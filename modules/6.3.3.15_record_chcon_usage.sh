...
DESCRIPTION="6.3.3.15 - Ensure successful and unsuccessful attempts to use the chcon command are recorded"

PACKAGE='auditd'
UID_MIN=$(awk '/^\s*UID_MIN/{print $2}' /etc/login.defs)
AUDIT_PARAMS="-a always,exit -F path=/usr/bin/chcon -F perm=x -F auid>=$UID_MIN -F auid!=unset -k perm_chng"
