...
PACKAGE='ufw'
UFW_PATTERNS='^Anywhere\s+ALLOW[[:space:]]OUT\s+Anywhere[[:space:]]on[[:space:]]all ^Anywhere[[:space:]]\(v6\)\s+ALLOW[[:space:]]OUT\s+Anywhere[[:space:]]\(v6\)[[:space:]]on[[:space:]]all'
UFW_RULES='ufw_allow_out_on_all'
...
apply() {
    for ufw_rule in $UFW_RULES; do
        ...
        if $rule 2>/dev/null | grep -q "added"; then
            fixd "Added rule for outbound traffic ($rule)"
        fi
    done
}
